...
def main():
    ...
    # calls training loop, w1 represents matrix to transform input poses to latent, w2 is the decoder matrix
    w1, w2 = train_acae(
...
def train_acae(
    ...
    left_ids = [i for i, name in enumerate(joint_names) if name[0] == 'l']  # groups all joint name idxs that start with l as left
    right_ids = [joint_names.index('r' + name[1:])
                 for i, name in enumerate(joint_names) if name[0] == 'l']  # ensures joint pairings on the right are in same order as they appear in left
    center_ids = [i for i, name in enumerate(joint_names) if name[0] not in 'lr']  # groups all joint idxs that don't start with l or r as center
    permutation = left_ids + right_ids + center_ids  # creates a list of joint indices in the order of left, right, center
    inv_permutation = tf.math.invert_permutation(permutation)  # finds original mapping of those indices for later, so we can restore original joint order
    ...
    return w1, w2  #(Joints, Latent), (Latent, Joints)
...
# trains autoencoder, called by trian_acae in main()
...
def splat(x, y):  #projects 3d into 2d with camera perspective assumptions
